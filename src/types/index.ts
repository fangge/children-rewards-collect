...
declare global {
  interface Window {
    electronAPI: {
      ...
      selectImage: () => Promise<{ canceled: boolean; filePath?: string; fileName?: string }>;
      
      // 数据导入导出
      exportData: () => Promise<{ success: boolean; error?: string; canceled?: boolean }>;
      importData: () => Promise<{ success: boolean; error?: string; canceled?: boolean }>;
    };
  }
}